...
trait ZFAxiom {
   ...
   */
  def replacement[φ[_, _, _, _]]: ∀[[A] => ∀[[p] => A hasAll ([x] => ∃![[y] => φ[x, y, A, p]]) => ∃[[Y] => A hasAll ([x] => Y hasSome ([y] => φ[x, y, A, p]))]]]
...
object ZFAxiom {
  ...
  def pairing(implicit axiom: ZFAxiom): ∀[[a] => ∀[[b] => ∃[[x] => (a ∈ x) ∧ (b ∈ x)]]] = axiom.pairing
